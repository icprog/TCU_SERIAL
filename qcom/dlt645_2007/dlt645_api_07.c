...
INT32 pack_d07_frame_by_data(S_D07_PACK_FRAME *inPara, char *outBuffer, INT32 *outLength)
{
    ...
    // 8 计数检验和
    for(i = 0; i < len; i++)//change by andy i = 0
    {
...
eD07Err trans_d07_data_recd_zone_prog(E_D07_TRANS_FLG flag, char *user, char *frame)    
{
    ...
    if(E_D07_TRANS_U2F == flag)
    {
        ...
        if(D07_ON == g_need_pack_07)
        {
            ...
            //my_operator
            sTest.my_operator[0] = 0x01;
            sTest.my_operator[1] = 0x02;
            sTest.my_operator[2] = 0x03;
            sTest.my_operator[3] = 0x04;
        ...
        sprintf(strUsr, 
                ...
                //my_operator
                pDataUser->my_operator[0],
                pDataUser->my_operator[1],
                pDataUser->my_operator[2],
                pDataUser->my_operator[3],
    ...
    else// frame to user interface
    {
        ...
        //my_operator
        pDataUser->my_operator[0] = d07_str_to_num(&strUsr[i+=2], 2);
        pDataUser->my_operator[1] = d07_str_to_num(&strUsr[i+=2], 2);
        pDataUser->my_operator[2] = d07_str_to_num(&strUsr[i+=2], 2);
        pDataUser->my_operator[3] = d07_str_to_num(&strUsr[i+=2], 2);
...
eD07Err trans_d07_data_recd_adjust_time(E_D07_TRANS_FLG flag, char *user, char *frame)    
{
    ...
    else// frame to user interface
    {
        ...
        //my_operator
        pDataUser->my_operator[0] = d07_str_to_num(&strUsr[i], 2);
        pDataUser->my_operator[1] = d07_str_to_num(&strUsr[i+=2], 2);
        pDataUser->my_operator[2] = d07_str_to_num(&strUsr[i+=2], 2);
        pDataUser->my_operator[3] = d07_str_to_num(&strUsr[i+=2], 2);
        //sBefTime
...
eD07Err trans_d07_data_recd_holiday_prog(E_D07_TRANS_FLG flag, char *user, char *frame)    
{
    ...
    else// frame to user interface
    {
        ...
        for(j = 0; j < 11; j++)
        {
            pDataUser->my_operator[j] = d07_str_to_num(&strUsr[i+=2], 2);
        }
